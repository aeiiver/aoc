...
#define issym(c)   (isdigit(c) || (c) == '.')

static int one(void)
{
    int sum = 0;

    for (int r = 0; r < arrlen(puzzle); r += 1) {
        for (int c = 0; c < arrlen(puzzle); c += 1) {
            if (!isdigit(puzzle[r][c])) {
                continue;
            }

            int n = atoi(&puzzle[r][c]);
            bool should_add = false;

            for (; c < arrlen(puzzle) && isdigit(puzzle[r][c]); c += 1) {
                if (should_add) {
                    continue;
                }

                int rs = r > 0 ? r - 1 : 0;
                int re = r < arrlen(puzzle) - 1 ? r + 1 : arrlen(puzzle) - 1;
                int cs = c > 0 ? c - 1 : 0;
                int ce = c < arrlen(puzzle) - 1 ? c + 1 : arrlen(puzzle) - 1;

                for (int r2 = rs; r2 <= re; r2 += 1) {
                    for (int c2 = cs; c2 <= ce; c2 += 1) {
                        if (r2 == r && c2 == c) {
                            continue;
                        }
                        if (issym(puzzle[r2][c2])) {
                            continue;
                        }
                        should_add = true;
                        break;
        ...
        for (int c = 0; c < arrlen(puzzle); c += 1) {
            ...
            for (int r2 = rs; r2 <= re; r2 += 1) {
                for (int c2 = cs; c2 <= ce; c2 += 1) {
                    ...
                    int c3 = c2;
                    for (; c3 >= 0 && isdigit(puzzle[r2][c3]); c3 -= 1) { }
                    ns[ns_nb++] = atoi(&puzzle[r2][c3 + 1]);

                    for (; c2 <= ce && isdigit(puzzle[r2][c2]); c2 += 1) { }
                }
                if (stop) {
                    break;
                }
